  ...
  <div>
    ...
    <!-- <h1>校园新闻列表</h1> -->
      <el-breadcrumb  separator-class="el-icon-arrow-right" separator="/">
        ...
        <el-breadcrumb-item :to="{ path: '/news/list' }">校友风采管理</el-breadcrumb-item>
      </el-breadcrumb>
      ...
      <el-table :data="items">
        ...
        <el-table-column prop="image" label="封面" width="260" >
        ...
        <el-table-column prop="excellentName" label="校友名字" >
        </el-table-column>
...
<script>
  export default {
    ...
    methods: {
      ...
      async fetch () {
        const res = await this.$http.get('rest/demeanor', {
          params: {
      ...
      edit (row) {
        this.$router.push(`/demeanor/edit/${row._id}`)
      },
      deleted (row) {
        this.$confirm(`此操作将删除校友风采《${row.title}》!`, '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
          }).then(async () => {
            await this.$http.delete(`rest/demeanor/${row._id}`)
            this.fetch()
            this.$message({
              type: 'success',
              message: '删除成功!'
